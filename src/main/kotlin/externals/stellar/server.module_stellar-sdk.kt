...
import Server.Timebounds
import Horizon.FeeStatsResponse
import Server.SubmitTransactionOptions
import Horizon.SubmitTransactionResponse

external open class Server(serverURL: String, opts: Options = definedExternally) {
    open var serverURL: Any
    open fun fetchTimebounds(seconds: Number, _isRetry: Boolean = definedExternally): Promise<Timebounds>
    open fun fetchBaseFee(): Promise<Number>
    open fun feeStats(): Promise<FeeStatsResponse>
    open fun submitTransaction(transaction: Transaction__0, opts: SubmitTransactionOptions = definedExternally): Promise<SubmitTransactionResponse>
    open fun submitTransaction(transaction: Transaction__0): Promise<SubmitTransactionResponse>
    open fun submitTransaction(transaction: FeeBumpTransaction, opts: SubmitTransactionOptions = definedExternally): Promise<SubmitTransactionResponse>
