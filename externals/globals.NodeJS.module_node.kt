...
external interface ReadableStream : events.global.NodeJS.EventEmitter {
    ...
    fun <T : WritableStream> pipe(destination: T, options: `T$6` = definedExternally): T
...
external interface RequireResolve {
    @nativeInvoke
    operator fun invoke(id: String, options: `T$7` = definedExternally): String
...
external interface ProcessConfig {
    var target_defaults: process.global.NodeJS.`T$8`
    var variables: process.global.NodeJS.`T$9`
}
...
external interface Process : events.global.NodeJS.EventEmitter {
    ...
    var features: process.global.NodeJS.`T$13`
    fun umask(): Number
    fun umask(mask: String): Number
    fun umask(mask: Number): Number
    fun uptime(): Number
    var hrtime: process.global.NodeJS.HRTime
    val send: ((message: Any, sendHandle: Any, options: process.global.NodeJS.`T$14`, callback: (error: Error?) -> Unit) -> Boolean)?
    fun disconnect()
